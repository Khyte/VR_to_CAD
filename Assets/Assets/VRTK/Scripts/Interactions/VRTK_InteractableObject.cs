...
namespace VRTK
{
	...
	[Tooltip("If this is checked then the interactable object script will be disabled when the object is not being interacted with. This will eliminate the potential number of calls the interactable objects make each frame.")]
        ...
        public virtual void OnInteractableObjectGrabbed(InteractableObjectEventArgs e, GameObject go)
        {
            if (InteractableObjectGrabbed != null)
            {
				InteractableObjectGrabbed(go, e);
			}
        }
        ...
        public virtual void Grabbed(GameObject currentGrabbingObject)
        {
			...
			/*firstTransform = gameObject.transform.position;
			firstRotation = gameObject.transform.localRotation;
			firstScale = gameObject.transform.localScale;				
			if (gameControl.GetComponent<CopyPaste>().copyMode) {
				GameObject cloneObject = Instantiate(gameObject, gameObject.transform.position, gameObject.transform.rotation);
				cloneObject.name = "Object" + gameControl.GetComponent<ExportOBJ>().numObject + "_" + gameObject.name + "_Copy";
				gameControl.GetComponent<ExportOBJ>().numObject++;
				int indexOfObj = gameControl.GetComponent<Historique>().listObjects.IndexOf(gameObject);
				if (indexOfObj >= 0) {
				...
				OnInteractableObjectGrabbed(SetInteractableObjectEvent(currentGrabbingObject), cloneObject);
			}*/
			//
			ToggleEnableState(true);
			if (!IsGrabbed() || IsSwappable()) {
        ...
        public virtual void Ungrabbed(GameObject previousGrabbingObject)
        {
			...
			//
			if (!gameControl.GetComponent<CopyPaste>().copyMode && !GetComponent<VRTK_AxisScaleGrabAction>().isScaling) {
				gameControl.GetComponent<Historique>().HistoCreate(gameObject, false, false);
			}
			else if (gameControl.GetComponent<CopyPaste>().copyMode) {
				gameControl.GetComponent<Historique>().HistoCreate(gameObject, true, false);
			}
			//
        ...
        public void PrimaryControllerGrab(GameObject currentGrabbingObject)
        {
        ...
        public void SecondaryControllerGrab(GameObject currentGrabbingObject)
        {
        ...
        public void PrimaryControllerUngrab(GameObject previousGrabbingObject)
        {
            UnpauseCollisions();
            RemoveTrackPoint();
		...
		public void SecondaryControllerUngrab(GameObject previousGrabbingObject)
        {
